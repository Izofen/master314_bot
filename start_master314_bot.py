...
def get_ask_user (message_info,status_input):
    import iz_bot
    namebot    = message_info.setdefault('namebot','')
    db,cursor = iz_bot.connect (namebot)
    sql = "select id,name,info,message_in,status_in,message_status from user_ask where 1=1 order by id ".format ()
    cursor.execute(sql)
    data = cursor.fetchall()
    list = []
    for rec in data: 
        id,name,info,message_out,status_in,message_status = rec.values() 
        if status_input.setdefault(info,'') == '':
            list.append(rec)
    return list  

def test_main_word (message_info):
    import iz_bot
    namebot    = message_info.setdefault('namebot','')
    message_in = message_info.setdefault('message_in','')
    db,cursor = iz_bot.connect (namebot)
    sql = "select id,name,info from menu where 1=1 ".format ()
    cursor.execute(sql)
    data = cursor.fetchall()
    label = True
    for rec in data: 
...
def test_main_key (message_info):
    import iz_bot
    message_in = message_info.setdefault('message_in','')
    list = ['/start']
    label = True
...
def start_prog (message_info):
    ...
    label = '0'
    if test_main_key (message_info) == True: 
        if status != '':
            ask_user = get_ask_user (message_info,status_input)
            for rec in ask_user: 
                id,name,info,message_out,status_in,message_status = rec.values() 
                if status == status_in:
                    if test_main_word (message_info) == True:
                        send_data = {"Text":message_status}
                        answer = iz_bot.send_message (message_info,send_data)                    
                        save_data = [[info,message_in],['status','']]        
                        iz_bot.user_save_data (message_info,save_data) 
                        status_input[info] = message_in
                        break
                else:    
                    send_data = {"Text":"Укажите правильное значение"}
                    answer = iz_bot.send_message (message_info,send_data)                 
        ask_user = get_ask_user (message_info,status_input)
        ...
        for rec in ask_user: 
            ...
            id,name,info,message_out,status_in,message_status = rec.values() 
            label = False
            send_data = {"Text":message_out}
            answer = iz_bot.send_message (message_info,send_data)   
            save_data = [['status',status_in]]        
            iz_bot.user_save_data (message_info,save_data) 
            message_in = ''
            break
            
    #if label == '1':
    #    message_in = 'Главное меню'
            
    if message_in   == 'Показать заказ':
        send_data = {"Text":'Показать'}
        answer = iz_bot.send_message (message_info,send_data)
    ...
    if message_in   == 'Создать заказ':
        iz_bot.deleteMessage (message_info,message_id)
        db,cursor = iz_bot.connect (namebot)
        param = {'limit':10,'offset':0}
        sql = "select id,info,data_id from service where name = 'Категория' and status <> 'delete'  ORDER BY id DESC limit #offset#,#limit#"        
        id_code = save_sql (message_info,sql,param)
        key_array = list_music (message_info,id_code,'new')
        print ('[+] key_array:',key_array)
        send_data = {"Text":'Категория','Кнопки':key_array,'Тип кнопки':'Сообщение'}
        answer = iz_bot.send_message (message_info,send_data)
